...
void report_status_message(uint8_t status_code)
{
  switch(status_code) {
    case STATUS_OK: // STATUS_OK
      ...
      printPgmString(PSTR("Command has been parsed successfully ")); // Message Added for Debuggint Purposes ( Must be REMOVED !!!!!)
      printPgmString(PSTR("ok\r\n"));
      

      /// MODIFICATION 

      // Now Giving the HIGH to LOW Pulse 

        PORTC = PORTC | (BIT4) ; // Lighting up the Analog Pin4 
        delay_sec(2, DELAY_MODE_DWELL);
        PORTC = PORTC & (~BIT4) ; // Lighting off the Analog Pin4 
        delay_sec(2, DELAY_MODE_DWELL);
        PORTC = PORTC | (BIT4) ; // Lighting up the Analog Pin4 
        delay_sec(2, DELAY_MODE_DWELL);
        PORTC = PORTC & (~BIT4) ; // Lighting of the Analog Pin4  


        // Above the delay is given in seconds , instead in reality it will be given as for milli second  / micro second with the help of 
        // function delay_ms or delay_us .  SO change will be made accordingly as per the requirement . 
...
void report_init_message()
{
  ...
  printPgmString(PSTR("Hii !! from Kartik Sikka "));
}
